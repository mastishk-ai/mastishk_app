...
class MastishkModelManager:
    ...
    def load_enhanced_checkpoint(
        ...
        try:
            ...
            if not checkpoint_id in self.checkpoint_manager.metadata:
                return self._load_legacy_checkpoint_with_progress(checkpoint_path)  # ← CHANGED
            ...
            if success:
                self.initialized = True
                # 🔥 NEW: Store training state for later restoration
                self.restored_training_state = training_state  # ← ADDED
                return True, message
            else:
    ...
    def _load_legacy_checkpoint_with_progress(self, checkpoint_path: str) -> Tuple[bool, str]:
        """Load legacy checkpoint and extract training progress"""
        try:
            print(f"📂 Loading legacy checkpoint with progress extraction from {checkpoint_path}")
            checkpoint = torch.load(checkpoint_path, map_location=self.device)
            
            # Load model state (same as before)
            if 'config_dict' in checkpoint:
                config_dict = checkpoint['config_dict']
                config = MastishkConfig(**config_dict)
                print("✅ Loaded config from dictionary (new format)")
            ...
            # 🔥 NEW: Extract training progress from legacy checkpoint
            extracted_progress = {
                'step': checkpoint.get('current_step', checkpoint.get('step', 0)),
                'epoch': checkpoint.get('current_epoch', checkpoint.get('epoch', 0)),
            ...
            # Store extracted progress for trainer restoration
            self.restored_training_state = extracted_progress
            
            timestamp = checkpoint.get('timestamp', 'Unknown')
            vocab_size = config.vocab_size
            hidden_size = config.hidden_size
            
            progress_info = f"Step: {extracted_progress['step']}, Epoch: {extracted_progress['epoch']}, Best Loss: {extracted_progress['best_loss']:.4f}"
            
            return True, f"✅ Legacy model loaded with progress!\nTimestamp: {timestamp}\nConfig: {hidden_size}h, {config.num_hidden_layers}l, vocab={vocab_size}\n{progress_info}"
...
class EnhancedTrainer:
    ...
    def train(
        ...
        # 🔥 FIXED: Use monitor step if available, fallback to resume_from_step
        if hasattr(self.monitor, 'step') and self.monitor.step > 0:
            step = self.monitor.step
            print(f"🔄 Resuming from monitor step: {step}")
        else:
            step = self.resume_from_step
            print(f"🔄 Starting from resume step: {step}")
        ...
        try:
            ...
            if resume_from_checkpoint:
                ...
                if success:
                    ...
                    # 🔥 NEW: Restore training progress to trainer's monitor
                    if hasattr(self.model_manager, 'restored_training_state'):
                        restored_state = self.model_manager.restored_training_state
                        
                        # Restore training monitor state
                        self.current_trainer.monitor.step = restored_state.get('step', 0)
                        self.current_trainer.monitor.best_loss = restored_state.get('best_loss', float('inf'))
                        self.current_trainer.monitor.train_losses = restored_state.get('train_losses', [])
                        self.current_trainer.monitor.learning_rates = restored_state.get('learning_rates', [])
                        self.current_trainer.monitor.gradient_norms = restored_state.get('gradient_norms', [])
                        
                        # Set resume point for trainer
                        self.current_trainer.resume_from_step = restored_state.get('step', 0)
                        
                        print(f"🔄 Restored training progress: Step {restored_state.get('step', 0)}, Best Loss: {restored_state.get('best_loss', 'N/A')}")
                        print(f"📊 Restored {len(restored_state.get('train_losses', []))} loss history entries")
                    else:
                        print("⚠️ No training progress found in checkpoint")
