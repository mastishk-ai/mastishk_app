        ...
        if torch.isnan(loss):
            ...
            for name, param in self.model.named_parameters():
                if param.grad is not None:
                    print(f"🔍 Gradient check — {name}: grad norm = {param.grad.norm().item():.6f}")
                else:
...
class EnhancedTrainerWithWeightVerification(EnhancedTrainer):
    ...
    def train_step_with_verification(self, batch: Dict[str, torch.Tensor], step: int) -> Dict[str, float]:
        """Enhanced training step with comprehensive weight verification"""
        ...
        if self.enable_weight_verification:
            ...
            pre_snapshot = self.weight_verifier.create_weight_snapshot(
                self.model, step, "pre_training_step", f"Before training step {step}"
            )
        
        # Perform the actual training step
        step_metrics = super().train_step(batch)
        
        if self.enable_weight_verification and step_metrics.get('loss', 0) > 0:
            # Post-training verification
            post_snapshot = self.weight_verifier.create_weight_snapshot(
                self.model, step, "post_training_step", f"After training step {step}, loss: {step_metrics.get('loss')}"
            )
            ...
            # Add verification metrics to step metrics
            step_metrics.update({
                'weights_updated': verification_results.get('weights_changed', False),
                'layers_changed': len(verification_results.get('layers_changed', [])),
                'verification_status': verification_results.get('verification_status', 'Unknown')
            ...
            if verification_results.get('weights_changed', False):
                print(f"✅ Step {step}: Weights updated successfully - {len(verification_results.get('layers_changed', []))} layers changed")
            else:
                print(f"❌ Step {step}: No weight updates detected!")
...
def render_enhanced_sidebar():
    ...
    with st.sidebar:
        ...
        with st.expander("🔧 Advanced Configuration", expanded=False):
            ...
            with col1:
                use_flash = st.checkbox("Flash Attention", value=False)
                use_moe = st.checkbox("Mixture of Experts", value=False)
                use_mod = st.checkbox("Mixture of Depths", value=False)
            
            with col2:
                use_minimax = st.checkbox("MiniMax Attention", value=False)
                use_quantization = st.checkbox("8-bit Quantization", value=False)
...
def render_enhanced_deployment_tab():
    ...
    if export_format == "Enhanced Checkpoint (.pt)" and selected_checkpoint_id:
        ...
        with col1:
            ...
            include_scheduler = st.checkbox("Include Scheduler State", value=False)
        with col2:
            include_training_state = st.checkbox("Include Training State", value=False)
            compress_model = st.checkbox("Compress Model", value=True)
...
def render_enhanced_experiment_tab():
    ...
    with st.form("enhanced_experiment"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            track_checkpoints = st.checkbox("Track Checkpoints", value=True)
            track_generation = st.checkbox("Track Generation Quality", value=True)
        with col2:
            track_metrics = st.checkbox("Track Custom Metrics", value=True)
            auto_backup = st.checkbox("Auto Backup Results", value=False)
...
def main():
    ...
    with tabs[4]:  # ← NOTE: This changed from tabs[3] to tabs[4]
        render_enhanced_experiment_tab()
    
    with tabs[5]:  # ← NOTE: This changed from tabs[4] to tabs[5]
